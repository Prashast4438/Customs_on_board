...
export default function Login() {
  ...
  const card = {
    ...
    background: mode === 'dark' ? '#fff' : 'rgba(22,32,55,0.97)',
    ...
    <>
      <div style={bg}>
        ...
        <div className="auth-container">
          ...
          <div style={card}>
            <h2 style={{textAlign:'center', color: mode === 'dark' ? '#000' : '#fff'}}>Login</h2>
            <form onSubmit={handleSubmit} style={{textAlign:'center'}}>
              <input name="email" type="email" placeholder="Email" value={form.email} onChange={handleChange} required style={input} />
              <input name="password" type="password" placeholder="Password" value={form.password} onChange={handleChange} required style={input} />
              <button type="submit" style={button}>Login</button>
              {error && <div style={{...errorBox, textAlign:'center'}}>{error}</div>}
            </form>
            <p style={{ color: mode === 'dark' ? '#000' : '#fff', textAlign: 'center' }}>Don&apos;t have an account? <a href="/register" style={link}>Register</a></p>
          </div>
        </div>
      </div>
      {/* Footer */}
